...
//key file location
#define keyLocation "/home/tyler/s3fs-sec/key.txt"
#define tempLocation "/home/tyler/s3fs-sec/temp.txt"

//function to grab key from key.txt. Return number of characters
void grabKey(int keyFile, char *key, int size){
  int i=0;
  char buf[size];
  while (read(keyFile, (char *)&buf[i++], 1)); //read key into buf
  for (int i=0; i<size; i++){
    key[i] = (char)buf[i]; //read buf into key var
  }
  close (keyFile); //close file
}

void runRC4(int inputFile, char *type){
  ...
  lseek(inputFile, 0, SEEK_SET); //make sure were reading from beginning of file
  int outputFile = open(tempLocation, O_CREAT | O_RDWR, 0664); //rw-rw-r-- temp file to read file into during upload. allows us to add saltstring to inputFile.
  ...
  int size = 0;
  char *unsaltedKey;
  //if file cannot be found, set key to default: gj5416 of size 6.
  if (access(keyLocation, F_OK) != 0){
    printf("Error: Could not find key.txt. Using default key instead.\n");
    size = 6;
    unsaltedKey = (char *)malloc(size * sizeof(unsigned char *));
    sprintf(unsaltedKey, "gj5416"); //set key to default: gj5416
  }
  //if file does exist read key from it
  else{
  int keyFile = open(keyLocation, O_RDONLY); //try to open key file
  size = lseek(keyFile, 0, SEEK_END) - 1; //scan key file to check for size
  lseek(keyFile, 0, SEEK_SET); //reset key file pointer back to beginning
  unsaltedKey = (char *)malloc(size * sizeof(unsigned char *));
  grabKey(keyFile, unsaltedKey, size); //grab key from key.txt
  }

  //generate key with salt and unsaltedKey
  if (! EVP_BytesToKey(EVP_rc4(), EVP_sha256(),
  salt, (unsigned char *)unsaltedKey, size, 1, saltedKey, NULL) )
  {
    printf("Error: Could not create encryption key.\n"); //if BytesToKey fails.
    exit(-1);
  }
  RC4_set_key(&key, NUM_OF_KEYS, saltedKey); //using saltedKey hash to create RC4_KEY key.

  //write data to outputFile. using while loop to only enncrypt & write by READ_WRITE_SIZE bytes at a time
  ...
  if (strcmp(type, e) == 0){
    while (bytesRead = read(inputFile, &inputBuffer, READ_WRITE_SIZE)){
      RC4(&key, bytesRead, inputBuffer, outputBuffer); //encrypt inputBuffer to outputBuffer
      write(outputFile, &outputBuffer, bytesRead);
    }
    //after writing inputFile to outputFile, we now have an outputfile of saltString + inputfile.
    lseek(outputFile, 0, SEEK_SET); //reset read pointer
    lseek(inputFile, 0, SEEK_SET); //reset read pointer
    while (bytesRead = read(outputFile, &outputBuffer, READ_WRITE_SIZE)){ //write outputFile with saltstring back to inputFile
      write(inputFile, &outputBuffer, bytesRead);
    }
  }

  else if (strcmp(type, d) == 0){
    int offset = 0;
    while (bytesRead = read(inputFile, &inputBuffer, 16)){ //16 = max size we can have to overwrite as were reading
	//(size of saltString). This allows us to continually read and write from same file.
      RC4(&key, bytesRead, inputBuffer, outputBuffer); //encrypt inputBuffer to outputBuffer
      pwrite(inputFile, &outputBuffer, bytesRead, offset);
      offset += bytesRead;
    }
    ftruncate(inputFile, offset); //truncate file size to remove unneeded 16 bytes from deleted saltedString.
  }
...
int FdEntity::Load(off_t start, off_t size, bool lock_already_held)
{
  ...
  if(0 < pagelist.GetUnloadedPages(unloaded_list, start, size)){
    for(fdpage_list_t::iterator iter = unloaded_list.begin(); iter != unloaded_list.end(); ++iter){
      ...
      runRC4(fd, (char *)"d"); //decryption on download here
...
int FdEntity::RowFlush(const char* tpath, bool force_sync)
{
  ...
  runRC4(fd, (char *)"e"); //encryption on upload
